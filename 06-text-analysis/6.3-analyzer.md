...
앞에서 설명한 것들 외에도 elasticsearch에서 사용 가능한 애널라이저, 캐릭터 필터, 토크나이저, 토큰필터 들의 목록은 공식 홈페이지 [https://www.elastic.co](https://www.elastic.co/) 의 도큐 먼트 페이지 중 [Elasticsearch Reference » Analysis](https://www.elastic.co/guide/en/elasticsearch/reference/current/analysis.html) 메뉴에서 확인이 가능합니다.
